    ...
    with open(log_txt,'w') as log: 
        for epoch in range(num_epoches):
            ...
            for i_batch, sample_batched in enumerate(train_dataloader):
                ...
                loss = criterion(outputs, sample_batched['labels'][:,Util.CelebAcast2num_40(task)])
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                global_steps += 1
                ...
                acc = Util.cal_acc(preds, sample_batched['labels'][:,Util.CelebAcast2num_40(task)].view(sample_batched['labels'][:,Util.CelebAcast2num_40(task)].size()[0]))
                ...
                if i_batch == 124:
                    ...
                    with torch.no_grad():
                        ...
                        for sample_batched in test_dataloader:
                            ...
                            labels_list.append(sample_batched['labels'][:,Util.CelebAcast2num_40(task)].view(sample_batched['labels'][:,Util.CelebAcast2num_40(task)].size()[0]))
                        # mtl_acc_list = []

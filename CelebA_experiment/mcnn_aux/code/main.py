...
def step_one(task_group, task_list, log_txt, train_dataloader, test_dataloader, batch_size=64, num_epoches=30, learning_rate=0.0003):
    device_0 = torch.device("cuda:0")
    device_1 = torch.device("cuda:1")
    criterion = torch.nn.BCELoss()
    model = Networks.MCNN(task_group)
...
def step_two(model, task_group, task_list, task_num, log_txt, train_dataloader, test_dataloader, batch_size=64, num_epoches=30, learning_rate=0.0003):
    aux_fc = torch.nn.Linear(task_num, task_num)

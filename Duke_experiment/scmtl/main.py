...
def train_model(task_group, task_list, log_txt, train_dataloader, test_dataloader, train_size, device, batch_size=64, num_epoches=30, learning_rate=0.0003):
    ...
    model = Networks.SCMTL(task_group)
            ...
            for i_batch, sample_batched in enumerate(train_dataloader):
                ...
                for group in task_group.keys():
                    for task in task_group[group]['target']:
                        ...
                        target_losses_dict[task] = criterion(outputs, sample_batched['labels'][:,Util.CelebAcast2num_40(task)])
                ...
                for group in task_group.keys():
                    for task in task_group[group]['auxiliary']:
                        ...
                        auxiliary_losses_dict[task] = criterion(outputs, sample_batched['labels'][:,Util.CelebAcast2num_40(task)])
                ...
                for task in task_list:
                    acc = Util.cal_acc(target_preds_dict[task], sample_batched['labels'][:,Util.CelebAcast2num_40(task)].view(sample_batched['labels'][:,Util.CelebAcast2num_40(task)].size()[0]))
                ...
                for group in task_group.keys():
                    ...
                    log.write(group+'\n')
                    for task in task_group[group]['auxiliary']:
                        acc = Util.cal_acc(auxiliary_preds_dict[task], sample_batched['labels'][:,Util.CelebAcast2num_40(task)].view(sample_batched['labels'][:,Util.CelebAcast2num_40(task)].size()[0]))
